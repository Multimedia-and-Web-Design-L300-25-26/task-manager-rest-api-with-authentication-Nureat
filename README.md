...
**Note:** Tests use the database configured in `.env.test`. Copy `.env.test.example` to `.env.test` and set `MONGO_URI` and `JWT_SECRET`. If using MongoDB Atlas, ensure your current IP is allowed in **Network Access** (or allow access from anywhere `0.0.0.0/0` for testing).
